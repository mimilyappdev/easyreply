...
import { defaultData } from './data/defaultData_en.js'
...
const tData = (obj) => (typeof obj === 'object' ? obj.ja || '' : obj || '')
...
document.getElementById('settings-import-input').addEventListener('change', (e) => {
  ...
  reader.onload = (ev) => {
    try {
      ...
      storage.setItem('er_data_en', JSON.stringify(data))
      location.reload()
    } catch {
...
document.getElementById('btn-add-direction').addEventListener('click', () => {
  ...
  openEditModal({
    ...
    onSave: ({ label }) => {
      const phase = getPhase(currentPhaseId)
      if (!phase) return
      const newDir = { id: genId('dir'), label: label, buttons: [] }
...
document.addEventListener('click', (e) => {
  ...
  if (action === 'edit-btn') {
    ...
    openEditModal({
      title: t('button.edit'),
      label: typeof btn.label === 'object' ? (btn.label.ja || '') : (btn.label || ''),
      text:  typeof btn.text  === 'object' ? (btn.text.ja  || '') : (btn.text  || ''),
      hasText: true,
      onSave: ({ label, text }) => {
  ...
  if (action === 'edit-phase') {
    ...
    openEditModal({
      title: t('button.edit'),
      label: typeof phase.label === 'object' ? (phase.label.ja || '') : (phase.label || ''),
      onSave: ({ label }) => {
        phase.label = label
        saveData()
        renderPhaseTabs()
...
// Standalone: hide lang toggle and EN modal fields
document.getElementById('btn-lang').style.display = 'none'
document.getElementById('modal-label-en')?.closest('label')?.style.setProperty('display', 'none')
document.getElementById('modal-text-en-row')?.style.setProperty('display', 'none')
